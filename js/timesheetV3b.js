...
function addRow(dayId){
  ...
  var cols = "";
     ...
     cols += '<td><input type="time" class="timeField" "name="timein' + ident + '" id="timein' + ident + '"\
             oninput="checkThenCalc('+ ident +')"/></td>';
...
function checkThenCalc(ident){
  document.getElementById("hrs" + ident).innerHTML = "";
  var formdate = document.getElementById("maindate").value;
  var dayId = ident.split("-",1);
  var dday = moment(formdate).add(dayId, 'day').format('Y-MM-DD');
  var timeIn = document.getElementById("timein" + ident).value;
  var timeOut = document.getElementById("timeout" + ident).value;
  if (formdate == ""){
    return;
  }
  else if (timeIn == "") {
    ...
    document.getElementById("hrs" + ident).innerHTML = timeDiff;
    //calcWrkHrs();
  }
}
...
//Calculate totals
  function calcHrs(){
    ...
    while(dayId < 7){
      ...
      var entriesDayX = Array.from(document.getElementById('entriesDay-' + dayId).innerHTML.split(","));
      entriesDayX.forEach(calcWrkHrs);

      //Hours entry calc loop
        function calcWrkHrs(item){
          //add if statement to ignore line if hours type is not "work"
          var wrkHrs = doucument.getElementById("hrs" + item).innerHTML;
          dailyHrs = dailyHrs + wrkHrs;
          var reg = 0;
          var ott = 0;
          var ot = 0;
          var dt = 0;
          ...
          if(reg4Wk + wrkHrs > 40){
            ott = (reg4Wk + wrkHrs) - 40;
            wrkHrs = wrkHrs - ott;
            ot = ot + ott;
          }
          reg = wrkHrs;
          document.getElementById("reg" + item).innerHTML = reg;
          document.getElementById("ot" + item).innerHTML = ot;
          document.getElementById("dt" + item).innerHTML = dt;
          dailyReg = dailyReg + reg;
          dailyOT = dailyOT + ot;
          dailyDT = dailyDt + dt;
        }
